...
# https://github.com/Lizhi-sjtu/DRL-code-pytorch/blob/8f767b99ad44990b49f6acf3159660c5594db77e/8.SAC/SAC-continuous.py

class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, hidden_width, max_action=1):
...
class ReplayBuffer(object):
	...
	def store(self, s, a, r, s_, dw):
		self.num_transition = self.num_transition + 1
		self.s[self.count] = s
		self.a[self.count] = a
		self.r[self.count] = r
...
class SACAgent(object):
	def __init__(self, args, state_dim, action_dim):
		...
		self.batch_size = args.batch_size  # 256  # batch size
		self.GAMMA = args.gamma  # 0.99  # discount factor
		self.TAU = args.tau  # 0.005  # Softly update the target network
		self.lr = args.learning_rate  # 3e-4  # learning rate
		self.adaptive_alpha = True  # Whether to automatically learn the temperature alpha
		self.max_action = 1
		self.replay_buffer = ReplayBuffer(args.capacity, state_dim, action_dim)
		...
		self.actor = Actor(state_dim, action_dim, self.hidden_width, self.max_action)
	...
	def test(self, state):
		a = self.choose_action(state, deterministic=True)
		return a

	def save(self, epoch, paramid, reward):
		r = str(epoch) + '_r' + str(np.floor(reward[-1]).astype(int))
		dir = './p' + str(paramid)
		os.makedirs(dir, exist_ok=True)
		act_dic = {
			'state_dict':self.actor.state_dict(),
			'optim':self.actor_optimizer.state_dict(),
		}
		cri_dic = {
			'state_dict':self.critic.state_dict(),
			'optim':self.critic_optimizer.state_dict(),
		}
		tar_dic = {
			'state_dict':self.critic_target.state_dict(),
			'alpha':self.alpha,
		}
		torch.save(act_dic, dir + '/actor_' + r+ '.pth')
		torch.save(cri_dic, dir + '/critic_' + r+ '.pth')
		try:
			torch.save(tar_dic, dir + '/Targetc_' + r+ '.pth')
		except:
			print('save target_critic failed')
		# torch.save(self.Q_net2.state_dict(), dir + '/Q_net2_' + r)
		np.save(dir+'/'+r, np.array(reward))
		print("====================================")
		print("\033[34m[" + r + "] Model has been saved...\033[0m")
		print("====================================")

	def load(self, modeldir, epoch):
		f_a = glob.glob(modeldir+'/' + 'actor_' + str(epoch) + '*.pth')
		f_c = glob.glob(modeldir+'/' + 'critic_' + str(epoch) + '*.pth')
		f_t = glob.glob(modeldir+'/' + 'Targetc_' + str(epoch) + '*.pth')
		act_load = torch.load(f_a[0])
		cri_load = torch.load(f_c[0])

		try:
			self.actor.load_state_dict(act_load['state_dict'])
			self.critic.load_state_dict(cri_load['state_dict'])
		except:
			self.actor.load_state_dict(act_load)
			self.critic.load_state_dict(cri_load)
		try:
			tar_load = torch.load(f_t[0])
			self.critic_target.load_state_dict(tar_load['state_dict'])
			self.actor_optimizer.load_state_dict(act_load['optim'])
			self.critic_optimizer.load_state_dict(cri_load['optim'])
			self.alpha = tar_load['alpha']
		except:
			print('load target_critic failed')
		print("====================================")
...
def evaluate_policy(env, agent):  # no use
...
def reward_adapter(r, env_index):  # no use
...
if __name__ == '__main__':  # no use, just reference
		...
		while not done:
			...
			if (total_steps + 1) % evaluate_freq == 0:
				...
				if evaluate_num % 10 == 0:
					np.save('./data_train/SAC_env_{}_number_{}_seed_{}.npy'.format(env_name[env_index], number, seed),
